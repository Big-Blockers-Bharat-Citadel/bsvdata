...
const reversedDummyTxId = Buffer.from(dummyTxId, "hex")
  .reverse()
  .toString("hex");
const sighashType2Hex = (s: { toString: (arg0: number) => any }) =>
  s.toString(16);
...
export function createInputFromPrevTx(
  tx: { id: any; outputs: { [x: string]: any } },
  outputIndex: number
) {
